...
bool vmx_spte_accessed(u64 spte)
{
    return is_accessed_spte(spte);
}
EXPORT_SYMBOL(vmx_spte_accessed);

bool vmx_spte_dirty(u64 spte)
{
    return is_dirty_spte(spte);
}
EXPORT_SYMBOL(vmx_spte_dirty);

int vmx_spte_present(u64 spte)
{
    return is_shadow_present_pte(spte);
}
EXPORT_SYMBOL(vmx_spte_present);

u64 spte_to_hpa(u64 spte)
{
    return spte & PT64_BASE_ADDR_MASK;
}
EXPORT_SYMBOL(spte_to_hpa);
